...
void RootMonitor::OnEvent(const eudaq::StandardEvent & ev) {
  ...
  if(reduce){
    ...
    std::cout << "This is the true nwf for event " << ev.GetEventNumber() << " du gsicht: " << nwf << std::endl;
    ...
    if(!_planesInitialized){
      ...
      std::cout << "Number of waveforms read/expected: " << myevent.getNWaveforms() << "/" << nwf << std::endl;
