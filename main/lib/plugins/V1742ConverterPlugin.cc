...
static const char* EVENT_TYPE = "V1742";


class V1742ConverterPlugin:public DataConverterPlugin {


public:
  virtual void Initialize(const Event & bore, const Configuration & cnf) {
	std::cout<<"V1742 Initialize"<<std::endl;
	//m_serialno = bore.GetTag("V1742_serial_no", (int)-1);
	//m_firmware = bore.GetTag("V1742_firmware_v", (int)-1);
	//m_timestamp = bore.GetTag("V1742_timestamp", (int)-1);
	m_range = 0.5; //TODO

}

  virtual bool GetStandardSubEvent(StandardEvent & sev, const Event & ev) const {
//	std::cout << "\nV1742: GetStandardSubEvent" << std::endl;
...
private:
  V1742ConverterPlugin() : DataConverterPlugin(EVENT_TYPE), m_serialno(-1), m_firmware(-1), m_range(0){
	std::cout<<"V1742ConverterPlugin Constructor"<<std::endl;}

  uint64_t m_timestamp;
  uint32_t m_serialno;
  float m_firmware, m_range;
  static V1742ConverterPlugin m_instance;

}; // class V1742ConverterPlugin

V1742ConverterPlugin V1742ConverterPlugin::m_instance;
} // namespace eudaq
