...
void FileWriterText::WriteEvent(const DetectorEvent & devent) {
	...
	if (firstEvent)
	{
		if (sev.NumPlanes()==0 && sev.NumWaveforms()==0)// only TLU Events
		{
			...
			*m_out<<"i_time_stamp;  TLU_trigger"<<std::endl;
			...
			if (sev.NumPlanes() > 0)
				...
				*m_out<<"i_time_stamp; id_wf; trig_cell; data"<<std::endl;
		}
	...
	for (size_t iwf = 0; iwf < sev.NumWaveforms(); iwf++) {
		const eudaq::StandardWaveform & wf = sev.GetWaveform(iwf);

		*m_out<<sev.GetTimestamp()<<"; ";
		*m_out<<wf.ID()<<"; ";
		*m_out<<wf.GetTriggerCell()<<"; ";
		*m_out<<"[";
		for (unsigned s = 0; s < wf.GetData()->size();s++)
			*m_out<<wf.GetData()->at(s)<<",";
		*m_out<<"]; ";
		*m_out<<std::endl;
	}
	*m_out<<std::endl;
